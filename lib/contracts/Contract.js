...
const ConnectWalletPopup_1 = __importDefault(
  require("../component//ConnectWalletPopup")
);
...
class Contract extends eventcontainer_1.default {
  ...
  findMethodABI(name) {
    return this.abi.filter(
      (abi) => abi.name === name && abi.type === "function"
    )[0];
  }
  async loadExtWalletContract() {
    if (
      (await ExtWallet_1.default.loadChainId()) !== Config_1.default.chainId
    ) {
      ...
      if (this.walletContract === undefined) {
        this.walletContract = ExtWallet_1.default.createContract(
          this.address,
          this.abi
        );
      }
  ...
  async runWalletMethodWithGas(methodName, gas, ...params) {
    if (ExtWallet_1.default.installed === true) {
      ...
      await Klip_1.default.runContractMethod(
        this.address,
        this.findMethodABI(methodName),
        params
      );
    } else {
      return new Promise(
        (resolve) => new ConnectWalletPopup_1.default(resolve)
      );
    }
  }
