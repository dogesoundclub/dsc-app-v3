        ...
        if (walletAddress !== undefined) {
            ...
            for (let i = 0; i < balance; i += 1) {
                const promise = async (index) => {
                    ...
                    const compoundBlocks = (await BiasCompoundContract_1.default.compoundBlocks(mateId.toNumber() + 10000)).toNumber();
                    if (compoundBlocks != 0 && currentBlock - compoundBlocks >= returnMixTime) {
                        const mateItem = new BMCSItem_1.default(mateId.toNumber() + 10000).appendTo(this.mateList);
