...
export default class GovernanceDetail implements View {
                        ...
                        click: async () => {
                            if (this.proposalId !== undefined) {
                                if (forRadio.domElement.checked === true) {
                                    await DSCGovernanceV3Contract.voteFor(this.proposalId, MateContract.address, this.mateList.selectedMateIds);
                                    ViewUtil.waitTransactionAndRefresh();
                                }
                                if (againstRadio.domElement.checked === true) {
                                    await DSCGovernanceV3Contract.voteAgainst(this.proposalId, MateContract.address, this.mateList.selectedMateIds);
                                    ViewUtil.waitTransactionAndRefresh();
                                }
                            }
                        },
    ...
    private async load(id: string) {
        ...
        const proposal = await DSCGovernanceV3Contract.getProposal(id);
        ...
        if (proposal.executed === true) {
            ...
            const result = await DSCGovernanceV3Contract.getResult(id);
            if (result === DSCGovernanceV3Contract.VOTING) {
                title.append(el(".result.voting", `- ${msg("GOVERNANCE_PROPOSAL_VOTING")} -`));
            } else if (result === DSCGovernanceV3Contract.CANCELED) {
                title.append(el(".result.canceled", `- ${msg("GOVERNANCE_PROPOSAL_CANCELED")} -`));
            } else if (result === DSCGovernanceV3Contract.RESULT_SAME) {
                title.append(el(".result.same", `- ${msg("GOVERNANCE_PROPOSAL_RESULT_SAME")} -`));
            } else if (result === DSCGovernanceV3Contract.RESULT_FOR) {
                title.append(el(".result.for", `- ${msg("GOVERNANCE_PROPOSAL_RESULT_FOR")} -`));
            } else if (result === DSCGovernanceV3Contract.RESULT_AGAINST) {
                title.append(el(".result.against", `- ${msg("GOVERNANCE_PROPOSAL_RESULT_AGAINST")} -`));
            } else if (result === DSCGovernanceV3Contract.AVOID) {
                title.append(el(".result.against", `- ${msg("GOVERNANCE_PROPOSAL_RESULT_AGAINST")} -`));
            }
        }
        ...
        voteCounts.append(el(".for-votes",
            ...
            el("span.count", String(await DSCGovernanceV3Contract.getForVotes(id))),
        ));
        voteCounts.append(el(".against-votes",
            el("span.title", msg("GOVERNANCE_PROPOSAL_VOTE_AGAINST")),
            el("span.count", String(await DSCGovernanceV3Contract.getAgainstVotes(id))),
        ));
        ...
        if (remains > 0) {
            ...
            if (walletAddress !== undefined) {
                        ...
                        el("a.cancel-button", msg("GOVERNANCE_CANCEL_PROPOSAL_BUTTON"), {
                            click: async () => {
                                new Confirm(msg("GOVERNANCE_CANCEL_PROPOSAL_WARNING"), msg("GOVERNANCE_CANCEL_PROPOSAL_BUTTON"), async () => {
                                    if (this.proposalId !== undefined) {
                                        await DSCGovernanceV3Contract.cancel(this.proposalId);
                                        ViewUtil.waitTransactionAndRefresh();
                                    }
                                });
                            },
                ...
                for (let i = 0; i < mateBalance; i += 1) {
                    const promise = async (index: number) => {
                        ...
                        mates.push(mateId);
                        if (await DSCGovernanceV3Contract.getMateVoted(id, MateContract.address, mateId) === true) {
            ...
            const walletAddress = await Wallet.loadAddress();
            if (walletAddress === proposal.proposer && await DSCGovernanceV3Contract.matesBacked(this.proposalId) !== true) {
                this.container.append(
                    el("a.get-back-mates-button", msg("GOVERNANCE_GET_BACK_MATES_BUTTON"), {
                        click: async () => {
                            if (this.proposalId !== undefined) {
                                await DSCGovernanceV3Contract.getBackMates(this.proposalId);
                                ViewUtil.waitTransactionAndRefresh();
                            }
                        },
                    }),
